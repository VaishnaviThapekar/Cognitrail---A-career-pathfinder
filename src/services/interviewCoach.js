...
✅ Use their names`;
    ...
    /**
     * Get mock interview questions for interactive session
     */
    getMockQuestionsForRole(role = 'Software Engineer') {
        const roleLower = role.toLowerCase();
        if (roleLower.includes('data') || roleLower.includes('analyst') || roleLower.includes('scientist')) {
            return [
                { id: 1, type: 'Behavioral', question: 'Tell me about a complex dataset you analyzed. What insights did you uncover and how did you present them?' },
                { id: 2, type: 'Technical', question: 'Explain the difference between supervised and unsupervised learning, giving a real-world example of each.' },
                { id: 3, type: 'Problem Solving', question: 'How would you handle missing or noisy data in a critical business predictive model?' }
            ];
        }
        if (roleLower.includes('design') || roleLower.includes('ux') || roleLower.includes('ui')) {
            return [
                { id: 1, type: 'Behavioral', question: 'Walk me through a design project where stakeholder feedback conflicted with user research.' },
                { id: 2, type: 'Design Method', question: 'How do you conduct usability testing and incorporate qualitative feedback into design iterations?' },
                { id: 3, type: 'System', question: 'How do you build and maintain an accessible component design system across web and mobile?' }
            ];
        }
        // Default Software Engineering / General Tech
        return [
            { id: 1, type: 'Behavioral', question: 'Describe a challenging technical problem you solved recently. What approach did you take?' },
            { id: 2, type: 'Technical', question: 'Explain how asynchronous operations, promises, and the event loop work in modern web development.' },
            { id: 3, type: 'System & Architecture', question: 'How do you design scalable RESTful or GraphQL APIs for high-concurrency client requests?' }
        ];
    }

    /**
     * Evaluate user answer locally when API is offline
     */
    evaluateAnswerLocally(questionText, userResponse) {
        ...
        if (wordCount < 15) {
            score -= 25;
            tips.push('Your answer is too short. Elaboration with specific examples will strengthen your response.');
        } else if (wordCount > 40) {
            score += 15;
            starFeedback.push('Good detail provided in setting up the context.');
        }

        const textLower = userResponse.toLowerCase();
        if (textLower.includes('result') || textLower.includes('outcome') || textLower.includes('%') || textLower.includes('improved') || textLower.includes('reduced')) {
            score += 15;
            starFeedback.push('Great use of quantifiable results or outcomes!');
        } else {
            tips.push('Consider adding measurable results (e.g. "improved performance by 20%").');
        }

        if (textLower.includes('because') || textLower.includes('first') || textLower.includes('then') || textLower.includes('finally')) {
            score += 10;
            starFeedback.push('Clear logical structure demonstrated.');
        }

        const finalScore = Math.min(95, Math.max(45, score));
        return {
            score: finalScore,
            starFeedback: starFeedback.length ? starFeedback : ['Response addresses the core question clearly.'],
            improvements: tips.length ? tips : ['Maintain this confident tone and pace during live interviews.']
        };
    }
}
