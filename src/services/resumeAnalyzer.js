...
5. Certifications/Projects`;
    ...
    /**
     * Local skill extraction & career database matching
     */
    extractSkillsAndMatch(resumeText) {
        const knownSkills = [
            'javascript', 'typescript', 'react', 'node.js', 'express', 'python', 'java', 'c++', 'c#', 'sql', 'mysql',
            ...
            'ui/ux', 'figma', 'photoshop', 'illustrator', 'cybersecurity', 'networking', 'linux', 'agile', 'scrum',
            'project management', 'communication', 'leadership', 'problem solving', 'teamwork', 'financial analysis',
            'marketing', 'seo', 'content writing', 'accounting', 'legal research', 'public speaking', 'biology', 'chemistry'
        ];

        const textLower = resumeText.toLowerCase();
        const detectedSkills = knownSkills.filter(skill => textLower.includes(skill));

        // Fallback skills if none detected
        const finalSkills = detectedSkills.length > 0 ? detectedSkills : ['communication', 'problem solving', 'project management'];

        // Calculate score
        const wordCount = resumeText.trim().split(/\s+/).length;
        const lengthScore = Math.min(30, Math.floor(wordCount / 10));
        const skillScore = Math.min(40, finalSkills.length * 5);
        const structureScore = textLower.includes('experience') || textLower.includes('education') ? 20 : 10;
        const totalScore = Math.min(98, Math.max(55, lengthScore + skillScore + structureScore));

        return {
            score: totalScore,
            detectedSkills: finalSkills.map(s => s.charAt(0).toUpperCase() + s.slice(1)),
            wordCount
        };
    }
}
