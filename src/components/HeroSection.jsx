...
import { Sparkles, ArrowRight, TrendingUp, Target, Zap, HelpCircle, FileText, Video } from 'lucide-react';
    ...
    <div className={`relative rounded-3xl overflow-hidden mb-16 transition-colors duration-500 ${darkMode ? 'bg-[#1a1d2e]' : 'bg-gradient-to-br from-blue-50 via-indigo-50 to-slate-50'
      ...
      <div className="relative grid md:grid-cols-2 gap-8 items-center p-12 md:p-16 lg:p-20">
        ...
        <div className="relative z-10">
          ...
          {/* Feature & Quick Action Pills matching original CSS UI */}
          <div className="flex flex-wrap gap-3 mb-10">
            ...
            {/* Quick Action Pills - Native Theme Styling */}
            <button
              onClick={() => window.openQuiz && window.openQuiz()}
              className={`flex items-center gap-2 px-4 py-2 rounded-full transition-all duration-300 transform hover:scale-105 cursor-pointer ${darkMode
                ? 'bg-[#272757]/30 hover:bg-[#272757]/60 border border-[#505081]/40 text-gray-200'
                : 'bg-amber-50 hover:bg-amber-100 border border-amber-200 text-amber-800 shadow-sm'
                }`}
            >
              <HelpCircle className={`w-4 h-4 ${darkMode ? 'text-amber-400' : 'text-amber-600'}`} />
              <span className="text-sm font-medium">Take AI Quiz</span>
            </button>

            <button
              onClick={() => window.openResume && window.openResume()}
              className={`flex items-center gap-2 px-4 py-2 rounded-full transition-all duration-300 transform hover:scale-105 cursor-pointer ${darkMode
                ? 'bg-[#0F0E47]/30 hover:bg-[#0F0E47]/60 border border-[#272757]/40 text-gray-200'
                : 'bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-indigo-800 shadow-sm'
                }`}
            >
              <FileText className={`w-4 h-4 ${darkMode ? 'text-indigo-400' : 'text-indigo-600'}`} />
              <span className="text-sm font-medium">Resume Matcher</span>
            </button>

            <button
              onClick={() => window.openInterview && window.openInterview()}
              className={`flex items-center gap-2 px-4 py-2 rounded-full transition-all duration-300 transform hover:scale-105 cursor-pointer ${darkMode
                ? 'bg-[#505081]/30 hover:bg-[#505081]/60 border border-[#8686AC]/40 text-gray-200'
                : 'bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-emerald-800 shadow-sm'
                }`}
            >
              <Video className={`w-4 h-4 ${darkMode ? 'text-emerald-400' : 'text-emerald-600'}`} />
